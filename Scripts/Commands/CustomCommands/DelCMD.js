...
module.exports.addcmd =  function (msg, client, content, cmd) {
    ...
    if (exists(cmd, msg)) {
        ...
        delete serverconfig.commands[cmd]
        fs.writeFileSync("Server/" + msg.guild.id + ".json", JSON.stringify(serverconfig));

        msg.channel.send(``, {
            embed: {
                title: ':thumbsup: The command has been removed successfully!',
                color: 0x2471a3,
                description: "",
                fields: [
                    {
        ...
        msg.channel.send("There isn't a command with this name!").then(msg => { msg.delete(5000) })
    }
